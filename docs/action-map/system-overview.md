...
| Journal   | Plan written and fsynced before step one, marked done after the last | None in the core crate. `event_store.rs` in the desktop crate is a SQLite WAL log with its own `reconcile_at_startup` (`event_store.rs:351`); only 8 of 46 writing commands use it                                                                                      | events-and-history.md                               |
...
| TreeHash  | A local git tree SHA matching GitHub and the CLI                     | None. `skill_content_hash` (`ops.rs:2137`) computes a content fingerprint capped at 2,000 files / 64 MiB, a different hash for a different purpose, not a git tree SHA                                                                                                  | install.md (implied, not built)                     |
...
| A second process writing the same root | No             | `ForkMutationLock` is in-process only; `FileLease` is cross-process-safe but wired only to `scan` (shared-state.md:41-43)                                                                                                                     |
...
| Per-root leases                                          | No            | `FileLease` exists (`lease.rs:17`) but every mutation command still uses the single global `ForkMutationLock`                                                                                 |
