...
| A number, such as "scan takes 1.9 s"                                  | a baseline        | a bench with a committed baseline file                                                     | no                      |
...
| 0.4 | Frontend timing: a dev-only overlay with IPC round trip and commit-to-paint per action, from `performance.mark`; a Vitest test that the snapshot reducer handles 400 skills under 5 ms                                                                                                   | overlay exists; test passes                                                                                            |
...
| 6.4 | Opt-in error reporting: off by default, one switch in Settings, the Sentry projects skill-studio-desktop-rust and skill-studio-desktop-react; envelopes pass a sanitizer in the core in the shape of #92: allow-listed operation names and dimensions, frames stripped to addresses or app-relative paths, no home path, skill name, or file body, caps on envelope size and queue depth, a bounded flush on quit | a test proves a home path, a skill name, and a SKILL.md body never leave the sanitizer; with the switch off, a test proves no network call; a forced panic in a dev build arrives symbolicated in the test project |
| 6.5 | Command health: a rollup of the timing log from 0.1 (count, p50, p95, failure count per command and per version) shown under Settings, Diagnostics, printed by `skill-studio doctor`, and attached to an error report only when 6.4 is on                                                                                                                                                                                | rollup equals a hand count over a fixture log; `doctor` prints it                                                                                                                                                     |
...
Every issue opens with a Measure table, filled before the work starts.
