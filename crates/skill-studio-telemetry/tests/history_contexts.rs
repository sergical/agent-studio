...
#[test]
fn history_workers_export_private_free_correlated_signals() {
    ...
    Hub::run(hub, || {
        tracing::subscriber::with_default(
            ...
            || {
                ...
                for (header, outcome) in [
                    (
                        "0123456789abcdef0123456789abcdef-0123456789abcdef-1",
                        "complete",
                    ),
                    (
                        "abcdef0123456789abcdef0123456789-abcdef0123456789-1",
                        "failed",
                    ),
                ] {
                    let context = ReadContext::capture_ipc(ReadOperation::History, Some(header));
                    ...
                    barrier.wait();
                    tracing::info!(target: "skill_studio_desktop::history", outcome, private_document = "PRIVATE_SENTINEL", duration_ms = 1.5, "skill.history.finished");
                })));
                }
    ...
    for log in &logs {
        assert_eq!(log["body"], "skill.history.finished");
    }
    for metric in &metrics {
        assert!(matches!(
            metric["name"].as_str(),
            Some("skill.history.count" | "skill.history.duration")
        ));
    }
    for expected in [
        ...
        assert!(transactions
            .iter()
            .any(|transaction| transaction["contexts"]["trace"]["trace_id"] == expected));
    }
    for transaction in &transactions {
        assert_eq!(transaction["spans"][0]["op"], "skill.history");
    }
    assert_ne!(
