...
#[test]
fn overlapping_read_contexts_keep_distinct_correlated_signals() {
    ...
    Hub::run(hub, || {
        tracing::subscriber::with_default(
            ...
            || {
                ...
                for operation in [ReadOperation::Scan, ReadOperation::Doctor] {
                    let context = ReadContext::capture(TelemetrySurface::Desktop, operation);
                    let barrier = barrier.clone();
                    workers.push(std::thread::spawn(move || context.run(|| {
                    let span = tracing::info_span!(target: "skill_studio_core::skill_service", "skill.scan");
