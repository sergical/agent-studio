...
//!
//! Ported from the desktop app's `skills/frontmatter.rs`. Pure functions over
//! `&str`; the core never touches a filesystem here. Diagnostics are
//! byte-identical to the desktop's so the projection stays a drop-in
//! replacement.
...
pub enum FrontmatterParseResult {
    /// The file has no `---` fence at all.
    Absent,
    /// The fence parsed as valid YAML.
    Valid(SkillFrontmatter),
    /// The fence is present but the YAML (or its closing fence) is broken.
...
pub struct FrontmatterParseError {
    ...
    pub column: usize,
    /// Human-readable message, with the YAML library's own line suffix
    /// stripped (the caller's own `line`/`column` replace it).
    pub message: String,
}

impl FrontmatterParseResult {
    /// Returns the parsed frontmatter, or `None` for [`Self::Absent`] and
    /// [`Self::Invalid`].
...
/// into an [`InvocationPolicy`], plus a spec-violation note when both keys
/// are set (they contradict each other) - `user-invocable: false` is dropped
/// in that case and `UserOnly` wins, since a skill unusable by either a
/// human or the model would be pointless.
...
/// [`invocation_policy`], taking the two raw key values directly rather than
/// a [`SkillFrontmatter`].
...
/// Parses a complete `SKILL.md` frontmatter fence without losing YAML errors.
...
/// Every top-level frontmatter key, stringified, for a caller that wants to
/// show agent-specific extensions the typed [`SkillFrontmatter`] doesn't
/// model. Malformed or missing frontmatter yields an empty map.
...
/// Validates `SKILL.md` frontmatter (plus overall file length) against the
